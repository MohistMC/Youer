...
public class PaperMinecraftSessionService extends YggdrasilMinecraftSessionService {
    ...
    public @Nullable ProfileResult fetchProfile(GameProfile profile, final boolean requireSecure) {
        ...
        new io.papermc.paper.event.profile.PreFillProfileEvent(playerProfile).callEvent();
        ...
        if (result != null) {
            new io.papermc.paper.event.profile.FillProfileEvent(CraftPlayerProfile.asBukkitMirror(result.profile())).callEvent();
        }
        return result;
    }
