...
public class WorldBackup {
    ...
    private static void doBackup() throws IOException {
        Path worldPath = Paths.get(MinecraftServer.getServer().getLevel(Level.OVERWORLD).bukkitName);
    ...
    private static void cleanupOldBackups(File backupDir) {
        ...
        File[] files = backupDir.listFiles((_, name) -> name.startsWith("world-") && (name.endsWith(".zip") || name.endsWith(".tar.zst") || name.endsWith(".zst")));
