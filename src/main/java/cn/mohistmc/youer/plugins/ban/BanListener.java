package cn.mohistmc.youer.plugins.ban;
...
import cn.mohistmc.youer.plugins.ban.utils.BanUtils;
    ...
    public static void save(InventoryCloseEvent event) {
        ...
        if (openInventory != null && openInventory.getInventory() == inventory) {
            if (openInventory.getBanType() == BanType.ITEM) {
                List<String> old = YouerConfig.ban_item_materials;
                ...
                List<String> old = YouerConfig.ban_entity_types;
                for (org.bukkit.inventory.ItemStack itemStack : event.getInventory().getContents()) {
                    if (itemStack != null && itemStack.getType() != Material.AIR) {
                ...
                List<String> old = YouerConfig.ban_enchantment_list;
                for (org.bukkit.inventory.ItemStack itemStack : event.getInventory().getContents()) {
                    if (itemStack != null && itemStack.getType() != Material.AIR) {
